...
class CSVExporterTest extends FunSpec with GeneratorDrivenPropertyChecks {
  describe("#export") {
    describe("given an empty dataset") {
      val dataset = new Dataset(List(), List())
    ...
    describe("given a dataset") {
      it("writes it as CSV") {
        val writer = new StringWriter()
        val exporter = new CSVExporter(writer)
        val dataset = new Dataset(List("hello", "sup"), List(new Entity(Map("hello" -> "world", "sup" -> "kid")), new Entity(Map("hello" -> "dude", "sup" -> "bro"))))
        exporter.export(dataset)
    ...
    describe("given a dataset with uneven attributes") {
      it("writes it as CSV") {
        val writer = new StringWriter()
        val exporter = new CSVExporter(writer)
        val dataset = new Dataset(List("hello", "sup"), List(new Entity(Map("hello" -> "world")), new Entity(Map("sup" -> "bro"))))
        exporter.export(dataset)
        val lines = writer.toString.trim.split("\r\n").toList
